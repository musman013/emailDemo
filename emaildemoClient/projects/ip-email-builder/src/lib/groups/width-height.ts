...
@Component({
  ...
  template: `
    <div class="group" [ngClass]="{'three': showAutoSlider()}">
      <mat-slide-toggle *ngIf="showAutoSlider()" [checked]="model.auto" (change)="toggleChange($event)">{{'GROUPS.WIDTH-HEIGHT.FIELDS.AUTO' | translate}}</mat-slide-toggle>
      <mat-form-field appearance="outline">
        <mat-label>{{label}}</mat-label>
        <input matInput [(ngModel)]="model.value" [disabled]="disableValueField()" type="number" placeholder="{{'GROUPS.WIDTH-HEIGHT.FIELDS.LABEL' | translate}}">
      </mat-form-field>
      <mat-form-field appearance="outline">
